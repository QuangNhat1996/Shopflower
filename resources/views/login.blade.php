            ...
            <div class="row">
                <div class="col-sm-4 col-sm-offset-1">
                    ...
                    <div class="login-form">
                        ...
                        <form method="POST" action="{{ url('/checklogin') }}">
                            ...
                            <input type="submit" class="btn btn-primary my-2" style="color: #fff" value="LOG IN"/>
                            @if(session('err1'))
                                <div class="alert alert-danger" style="color:rgb(220, 30, 1);">
                                {{session('err1')}}
                                </div>
                            @elseif(session('success_reset'))
                                <div class="alert alert-success">
                                    {{session('success_reset')}}
                                </div>
                            @endif
                        </form>
                        <a href="{{url('/forgot-password')}}" style="margin-top: 20%;">Forgot your password?</a>
                    </div>
                    <!--/login form-->
                </div>
                <div class="col-sm-1">
                ...
                <div class="col-sm-4">
                    <div class="signup-form">
                        ...
                        @endif
                        <form method="POST" action="{{ url('/createacc') }}">
                            @csrf
                            {{-- NHẬP FIRST NAME --}}
                            <input type="text" id="first_name" name="firstname" placeholder="First Name" required onkeyup="validateFirstName()"/>
                            <p style="color:rgb(163, 36, 17)" id="errFirstName"></p>

                            {{-- NHẬP LAST NAME --}}
                            <input type="text" id="last_name" name="lastname" placeholder="Last Name" required onkeyup="validateLastName()"/>
                            <p style="color:rgb(163, 36, 17)" id="errLastName"></p>

                            {{-- NHẬP SỐ ĐIỆN THOẠI --}}
                            <input type="tel" placeholder="Phone Number" id="phone_number" name="phonenumber" required/>
                            <p style="color:rgb(163, 36, 17)" id="errPhoneNumber"></p>

                            {{-- NHẬP EMAIL --}}
                            <input type="email" id="user_email" name="email" placeholder="Email" required onkeyup="validateEmail()"/>
                            <p style="color:rgb(163, 36, 17)" id="errEmail"></p>

                            {{-- NHẬP PASSWORD --}}
                            <input type="password" id="user_password" name="password" placeholder="Password" required onkeyup="validatePassword()"/>
                            <p style="color:rgb(163, 36, 17)" id="errPassword"></p>

                            {{-- NHẬP LẠI PASSWORD --}}
                            <input type="password" id="user_retype_password" name="password_confirmation" placeholder="Reconfirm Password" required onkeyup="validateRetypePassword()"/>
                            ...
                            <input type="submit" class="btn btn-primary my-2" style="color: #fff" value="REGISTER" id="registerButton"/>
                            @if(session('message'))
                                <div class="alert alert-success">
                                {{session('message')}}
                                </div>
